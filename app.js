...
// Middleware to extract userId from request body
app.use((req, res, next) => {
  // Get userId from request body
  req.userId = req.body.userId;
  if (!req.userId || typeof req.userId !== 'string') {
    return res.status(401).json({ error: 'Missing or invalid userId. Please include userId in the request body as JSON.' });
  }
  next();
});
...
app.get('/nutrition', (req, res) => {
  res.json({
    ...
    usage: 'POST /nutrition with JSON body: {"userId": "123", "foodName": "apple"}',
    examples: {
      simple: 'apple',
      pieces: '2 apples', 
      grams: 'I had 200 grams of rice',
  ...
  res.json({
    ...
    endpoints: {
      nutrition: {
        ...
        body: { userId: 'string', foodName: 'string (e.g., "2 apples", "200g rice", "1.5kg chicken")' }
      },
      cart: {
    ...
    examples: {
      simple: 'POST /nutrition {"userId": "123", "foodName": "apple"}',
      quantity: 'POST /nutrition {"userId": "123", "foodName": "2 apples"}',
      grams: 'POST /nutrition {"userId": "123", "foodName": "200 grams of rice"}',
      kilograms: 'POST /nutrition {"userId": "123", "foodName": "1.5 kg chicken"}'
    },
    features: [
